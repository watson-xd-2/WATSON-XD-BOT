...
async function gptOnline(message) {
    ...
    try {
        ...
        return res.data?.data || '❌ No response from AI.'
    } catch (err) {
...
let handler = async (m, { text, usedPrefix, command, conn }) => {
    ...
    const buttons = [
        { buttonId: `${usedPrefix}${command} Explain in simple words`, buttonText: { displayText: '📘 Explain Simply' }, type: 1 },
        { buttonId: `${usedPrefix}${command} Give an example`, buttonText: { displayText: '✏️ Give Example' }, type: 1 },
        { buttonId: `${usedPrefix}${command} Summarize`, buttonText: { displayText: '📝 Summarize' }, type: 1 }
    ]
