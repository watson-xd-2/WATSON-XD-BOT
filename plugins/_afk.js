let handler = (m) => m;

handler.before = (m) => {
  let user = global.db.data.users[m.sender];

  // Check if the sender was AFK
  if (user.afk > -1) {
    m.reply(
      `
You are no longer AFK${user.afkReason ? " after: " + user.afkReason : ""}
AFK duration: ${clockString(new Date() - user.afk)}
`.trim()
    );
    user.afk = -1;
    user.afkReason = "";
  }

  // Check if mentioned or quoted users are AFK
  let jids = [
    ...new Set([
      ...(m.mentionedJid || []),
      ...(m.quoted ? [m.quoted.sender] : []),
    ]),
  ];

  for (let jid of jids) {
    let user = global.db.data.users[jid];
    if (!user) continue;

    let afkTime = user.afk;
    if (!afkTime || afkTime < 0) continue;

    let reason = user.afkReason || "";
    m.reply(
      `
Do not tag this user!
They are currently AFK${reason ? " with reason: " + reason : " without reason"}
AFK duration: ${clockString(new Date() - afkTime)}
`.trim()
    );
  }

  return true;
};

export default handler;

// Converts milliseconds into hh:mm:ss format
function clockString(ms) {
  let h = isNaN(ms) ? "--" : Math.floor(ms / 3600000);
  let m = isNaN(ms) ? "--" : Math.floor(ms / 60000) % 60;
  let s = isNaN(ms) ? "--" : Math.floor(ms / 1000) % 60;
  return [h, m, s].map((v) => v.toString().padStart(2, 0)).join(":");
}